...
function OverviewChart({
  ...
  const area = `${line} L ${nx(series.length - 1)} ${padding + graphH} L ${nx(
    0
  )} ${padding + graphH} Z`;
...
function useTxHistory() {
  ...
  React.useEffect(
    () => PaymentsBus.subscribe((t) => setTx((p) => [t, ...p])),
    []
  );
  return { tx };
}
      ...
      >
        ...
        <RNAnimated.View style={{ transform: [{ scale: heroScale }] }}>
          ...
          <ScrollView
            ...
            snapToInterval={W * 0.78}
            decelerationRate="fast"
            contentContainerStyle={{
              paddingHorizontal: 16,
              paddingTop: 16,
              paddingBottom: 28, // more room so shadows show
            }}
          >
            {accounts.map((acc, idx) => (
              <View
                key={idx}
                style={{ width: W * 0.78, marginRight: 14, marginVertical: 6 }}
              >
                    ...
                    >
                      ...
                      <View style={[styles.rowBetween, { marginTop: 8 }]}>
                        ...
                        <View style={{ alignItems: "flex-end" }}>
                          ...
                          <Text style={styles.holderLight}>{CARD_HOLDER}</Text>
                        </View>
                      </View>
                    </LinearGradient>
                  </View>
        ...
        >
          ...
          <FlatList
            ...
            renderItem={({ item }) => {
              ...
              const icon = item.merchant.toLowerCase().includes("netflix") ? (
                <MaterialCommunityIcons name="netflix" size={18} />
              ) : item.merchant.toLowerCase().includes("paypal") ? (
                <MaterialCommunityIcons name="paypal" size={18} />
              ) : item.merchant.toLowerCase().includes("spotify") ? (
                <MaterialCommunityIcons name="spotify" size={18} />
              ) : (
                ...
                >
                  ...
                  <View style={{ padding: 14 }}>
                    <View style={styles.rowBetween}>
                      ...
                      >
                        <View style={styles.brandCircle}>{icon}</View>
                        <View>
